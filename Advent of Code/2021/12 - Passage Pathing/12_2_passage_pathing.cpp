...
std::int32_t CountPaths(const std::unordered_set<std::string_view>& visited, const Cave* current, const Cave* start,
                        const Cave* target, bool twice) {
  if (current == start && visited.contains(current->name))
    return 0;
  if (current == target)
    return 1;
  ...
  for (const auto neighbor : current->neighbors) {
    sub_paths += CountPaths(sub_visited, neighbor, start, target, twice);
  }
  return sub_paths;
}
...
int main() {
  ...
  const std::int32_t paths = CountPaths(visited, g.at("start").get(), g.at("start").get(), g.at("end").get(), false);
  std::cout << paths << '\n';
}
