...
vector<vector<int>> Expand(const vector<vector<int>>& risk) {
  const int n = static_cast<int>(risk.size());
  const int m = static_cast<int>(risk[0].size());
  vector<vector<int>> expanded(n * 5, vector<int>(m * 5, 0));
  for (int i = 0; i < n * 5; ++i) {
    for (int j = 0; j < m * 5; ++j) {
      const int original_risk = risk[i % n][j % m];
      const int r = i / n;
      const int c = j / m;
      expanded[i][j] = (original_risk + r + c - 1) % 9 + 1;
    }
  }
  return expanded;
}
...
int Solve() {
  const vector<vector<int>> risk = Expand(ReadInput());
  const int n = static_cast<int>(risk.size());
  const int m = static_cast<int>(risk[0].size());
