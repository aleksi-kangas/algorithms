...
void Expand(std::vector<std::vector<std::int32_t>>& risks, std::size_t factor) {
  const std::size_t n = risks.size();
  const std::size_t m = risks[0].size();
  for (auto& r : risks) {
    r.resize(m * factor, -1);
  }
  risks.resize(n * factor, std::vector<std::int32_t>(m * factor, -1));
  for (std::size_t i = 0; i < n * factor; ++i) {
    for (std::size_t j = 0; j < m * factor; ++j) {
      if (i < n && j < m)
        continue;
      const std::size_t tile_dy = i / n;
      const std::size_t tile_dx = j / m;
      const std::int32_t original_risk = risks[i % n][j % m];
      risks[i][j] = (original_risk + static_cast<std::int32_t>(tile_dy) + static_cast<std::int32_t>(tile_dx) - 1) % 9 + 1;
    }
  }
}
