...
position_t SpaceMap::RealPosition(const position_t& p) const {
  ...
  return {p.first + (empty_row_count_it != empty_row_count.end() ? empty_row_count_it->second : 0),
          p.second + (empty_column_count_it != empty_column_count.end() ? empty_column_count_it->second : 0)};
}
...
std::size_t SpaceMap::PairWiseDistance(const position_t& lhs, const position_t& rhs) const {
  ...
  return std::abs(static_cast<std::int32_t>(real_lhs.first) - static_cast<std::int32_t>(real_rhs.first)) +
         std::abs(static_cast<std::int32_t>(real_lhs.second) - static_cast<std::int32_t>(real_rhs.second));
}
