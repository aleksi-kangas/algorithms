...
struct Node {
  ...
  [[nodiscard]] bool IsStart() const { return id.ends_with('A'); }

  [[nodiscard]] bool IsEnd() const { return id.ends_with('Z'); }
...
int main() {
  ...
  for (const auto& node : nodes) {
    if (!node.IsStart())
      continue;
    distances.push_back(StepsToReachEnd(&node, instructions));
  }
  std::cout << std::accumulate(std::next(distances.begin()), distances.end(), static_cast<std::int64_t>(distances.front()),
