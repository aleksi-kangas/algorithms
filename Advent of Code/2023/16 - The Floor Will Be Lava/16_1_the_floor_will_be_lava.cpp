...
struct Contraption {
  ...
  [[nodiscard]] std::size_t EnergizedCount() const;
...
std::size_t Contraption::EnergizedCount() const {
  std::unordered_set<Beam> cache{};
  std::vector energized(tiles.size(), std::vector(tiles[0].size(), false));
  std::queue<Beam> q{};
  q.emplace(Beam{.position = {0, -1}, .direction = Beam::Direction::kRight});
  while (!q.empty()) {
...
int main() {
  ...
  std::cout << contraption.EnergizedCount() << std::endl;
}
