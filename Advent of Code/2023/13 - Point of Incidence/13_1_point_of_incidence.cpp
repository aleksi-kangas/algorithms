...
struct PatternNotes {
  ...
  [[nodiscard]] std::optional<std::size_t> FindHorizontalReflection() const;
  [[nodiscard]] std::optional<std::size_t> FindVerticalReflection() const;

  [[nodiscard]] static std::optional<PatternNotes> Parse(std::istream& is);
};

std::optional<std::size_t> PatternNotes::FindHorizontalReflection() const {
  ...
  for (std::size_t row{1}; row < pattern.size(); ++row) {
    ...
    for (std::size_t column{0}; column < pattern.front().size(); ++column) {
      std::size_t l{row - 1};
      std::size_t r{row};
      while (l > 0 && r < pattern.size() - 1 && pattern[l][column] == pattern[r][column]) {
        --l;
        ++r;
      }
...
std::optional<std::size_t> PatternNotes::FindVerticalReflection() const {
  if (pattern.empty() || pattern.front().empty() || pattern.size() == 1 || pattern.front().size() == 1)
    return std::nullopt;
  for (std::size_t column{1}; column < pattern.front().size(); ++column) {
    ...
    for (std::size_t row{0}; row < pattern.size(); ++row) {
      ...
      std::size_t r{column};
      while (l > 0 && r < pattern.front().size() - 1 && pattern[row][l] == pattern[row][r]) {
        --l;
        ++r;
      }
...
int main() {
  ...
  while (true) {
    ...
    if (!pattern_notes.has_value())
      break;
    answer += pattern_notes->FindHorizontalReflection().value_or(0) * 100;
    answer += pattern_notes->FindVerticalReflection().value_or(0);
  }
