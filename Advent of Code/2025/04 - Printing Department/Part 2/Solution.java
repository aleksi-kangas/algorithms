...
public final class Solution {
  ...
  private record Coordinates(int y,
                             int x) {
  ...
  public int removeAccessiblePapers() {
    int totalRemovedPaperCount = 0;
    final Set<Coordinates> cellsToBeRemoved = new HashSet<>();
    do {
      cellsToBeRemoved.clear();
      for (int y = 0; y < grid.size(); ++y) {
        for (int x = 0; x < grid.getFirst().size(); ++x) {
          if (canRemove(y, x)) {
            cellsToBeRemoved.add(new Coordinates(y, x));
          }
        }
      }
      for (final Coordinates coordinates : cellsToBeRemoved) {
        grid.get(coordinates.y).set(coordinates.x, Cell.EMPTY);
      }
      totalRemovedPaperCount += cellsToBeRemoved.size();
    } while (!cellsToBeRemoved.isEmpty());
    return totalRemovedPaperCount;
  }

  private boolean canRemove(final int y, final int x) {
    return grid.get(y).get(x) == Cell.PAPER && getNeighboringCellCount(y, x, Cell.PAPER) < 4;
  }
  ...
  static void main(final String[] args) {
    ...
    System.out.println(solution.removeAccessiblePapers());
  }
}