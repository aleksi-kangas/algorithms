...
class Solution {
  ...
  private static final class SeatMap {
    ...
    private int VisibleOccupiedCount(final int r, final int c) {
      int count = 0;
    ...
    private boolean CanSeeOccupied(final int r, final int c, final int dr, final int dc) {
      int nr = r + dr;
      int nc = c + dc;
      while (0 <= nr && nr < seatMap.size() && 0 <= nc && nc < seatMap.getFirst()
          .size()) {
        final Position positionSeen = seatMap.get(nr).get(nc);
        if (positionSeen == Position.OCCUPIED) {
          return true;
        }
        if (positionSeen == Position.EMPTY) {
          return false;
        }
        nr += dr;
        nc += dc;
