...
public final class Solution {
  ...
  public static final class TopographicMap {
    ...
    public int trailHeadScore(final Coordinates trailHead) {
      final Set<Coordinates> visited = new HashSet<>();
      final Queue<Coordinates> q = new ArrayDeque<>();
      visited.add(trailHead);
      q.add(trailHead);
      int score = 0;
      while (!q.isEmpty()) {
        final Coordinates c = q.poll();
        final int h1 = get(c);
        if (h1 == 9) {
          ++score;
        } else {
  ...
  public static void main(final String[] args) {
    ...
    for (final TopographicMap.Coordinates trailHead : trailHeads) {
      sum += topographicMap.trailHeadScore(trailHead);
    }
